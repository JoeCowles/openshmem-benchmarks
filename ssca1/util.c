...
shmem_team_t shmem_team_world(void);
int shmem_broadcast(shmem_team_t team, void *dest, const void *source, size_t nelems, int PE_root);
#endif
...
void distribute_rng_seed(unsigned int new_seed){
  ...
  shmem_broadcast(shmem_team_world(), &random_seed, &new_seed, 1, 0);
#endif
}
