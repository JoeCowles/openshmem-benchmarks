...
#if defined(USE_MANUAL_AGG) || defined(USE_CTX_OVERLAP)
/* ------------------------------------------------------------------------
 * Manual message aggregation (compile with -DUSE_MANUAL_AGG).
 *
 * The naive inner loop issues one 1-element SHORT_GET/PUT per cell per array.
 * These helpers instead transfer a whole contiguous index range in ONE bulk
 * SHORT_GET/PUT per owning PE (strip-mining), exactly mirroring what the
 * --openshmem-scalar-to-bulk compiler pass produces.  A distributed array is
 * block-partitioned: global index c lives on PE c/L at local offset c%L
 * (matrices add a row offset row*L, where L = local_length).
 * ---------------------------------------------------------------------- */

/* gather A[row][lo..hi] (global columns) into buf[0 .. hi-lo] */
static void agg_get_matrix(const score_matrix_t *A, index_t row,
                           index_t lo, index_t hi, score_t *buf) {
  const index_t Lc = A->local_length;
  index_t c = lo;
  while (c <= hi) {
    int pe = (int)(c / Lc);
    index_t boundary = (index_t)(pe + 1) * Lc - 1;
    index_t chi = boundary < hi ? boundary : hi;
    index_t count = chi - c + 1;
    index_t local = c - (index_t)pe * Lc;          /* c % Lc */
    SHORT_GET(buf + (c - lo), &(A->scores[index2d(row, local, Lc)]), (int)count, pe);
    c = chi + 1;
  }
}

/* scatter buf[0 .. hi-lo] to A[row][lo..hi] (global columns) */
static void agg_put_matrix(score_matrix_t *A, index_t row,
                           index_t lo, index_t hi, score_t *buf) {
  const index_t Lc = A->local_length;
  index_t c = lo;
  while (c <= hi) {
    ...
    index_t chi = boundary < hi ? boundary : hi;
    index_t count = chi - c + 1;
    index_t local = c - (index_t)pe * Lc;
    SHORT_PUT(&(A->scores[index2d(row, local, Lc)]), buf + (c - lo), (int)count, pe);
    c = chi + 1;
  }
}

/* gather sequence codons in->sequence[lo..hi] into buf[0 .. hi-lo] */
static void agg_get_seq(const seq_t *in, index_t lo, index_t hi, codon_t *buf) {
  const index_t Lc = in->local_size;
  short *typed = (short *)in->sequence;
  index_t c = lo;
  while (c <= hi) {
    int pe = (int)(c / Lc);
    index_t boundary = (index_t)(pe + 1) * Lc - 1;
    ...
    index_t count = chi - c + 1;
    index_t local = c - (index_t)pe * Lc;
    SHORT_GET((short *)(buf + (c - lo)), &typed[local], (int)count, pe);
    c = chi + 1;
  }
}

#ifdef USE_CTX_OVERLAP
/* ------------------------------------------------------------------------
 * Communication/computation overlap via OpenSHMEM contexts (-DUSE_CTX_OVERLAP).
 *
 * For anti-diagonal idx, three of the five gathered inputs are already final
 * while idx is being computed: the two read-only sequences and the score row
 * score[(idx-2)%3] (produced two diagonals earlier). We prefetch THOSE for
 * idx+1 on a dedicated context while computing idx, then complete that stream
 * with CTX_QUIET — hiding the read latency behind compute. The two gap arrays
 * (depth-1, produced at idx) stay on the critical path. Built on top of the
 * strip-mining helpers above; the prefetch variants are non-blocking on `ctx`.
 * ---------------------------------------------------------------------- */

/* non-blocking strip-mined gather of A[row][lo..hi] on context `ctx` */
static void agg_get_matrix_ctx(shmem_ctx_t ctx, const score_matrix_t *A,
                               index_t row, index_t lo, index_t hi, score_t *buf) {
  const index_t Lc = A->local_length;
  index_t c = lo;
  while (c <= hi) {
    int pe = (int)(c / Lc);
    ...
    index_t count = chi - c + 1;
    index_t local = c - (index_t)pe * Lc;
    CTX_SHORT_GET_NB(ctx, buf + (c - lo), &(A->scores[index2d(row, local, Lc)]), (int)count, pe);
    c = chi + 1;
  }
}

/* non-blocking strip-mined gather of sequence codons in->sequence[lo..hi] on `ctx` */
static void agg_get_seq_ctx(shmem_ctx_t ctx, const seq_t *in, index_t lo, index_t hi,
                            codon_t *buf) {
  const index_t Lc = in->local_size;
  short *typed = (short *)in->sequence;
  index_t c = lo;
  while (c <= hi) {
    ...
    index_t count = chi - c + 1;
    index_t local = c - (index_t)pe * Lc;
    CTX_SHORT_GET_NB(ctx, (short *)(buf + (c - lo)), &typed[local], (int)count, pe);
    c = chi + 1;
  }
}

/* The local column range [ls,le] this PE computes on anti-diagonal d. Replicates
 * the inline score_start/score_end -> local_start/local_end logic so the prefetch
 * for d+1 can be issued before d's body runs. Empty range => ls>le. */
static void ctx_diag_range(index_t d, index_t mlen, index_t lms, index_t lme,
                           index_t *ls, index_t *le) {
  index_t ss = d > (mlen - 1) ? (d - (mlen - 1)) : 0;
  index_t se = d < (mlen - 1) ? d : (mlen - 1);
  if (d < mlen) { ss++; se--; }
  if (se < lms && ss > lme) { *ls = 1; *le = 0; }
  else {
    *ls = lms > ss ? lms : ss;
    *le = se > lme ? lme : se;
  }
...
/* Prefetch the three reusable inputs (main_seq, match_seq, score row) for
 * anti-diagonal d over its range [lsd,led] into the given buffers, on `ctx`. */
static void ctx_prefetch_inputs(shmem_ctx_t ctx, index_t d, index_t lsd, index_t led,
                                const seq_t *main_seq, const seq_t *match_seq,
                                const score_matrix_t *score_matrix,
                                codon_t *bmain, codon_t *bmatch, score_t *bscore) {
  agg_get_seq_ctx(ctx, main_seq, lsd, led, bmain);                 /* main_seq[m]            */
  agg_get_seq_ctx(ctx, match_seq, d - led, d - lsd, bmatch);       /* match_seq[d-m]         */
  agg_get_matrix_ctx(ctx, score_matrix, (d - 2) % 3, lsd - 1, led - 1, bscore); /* score[(d-2)%3][m-1] */
}
#endif /* USE_CTX_OVERLAP */
#endif /* USE_MANUAL_AGG || USE_CTX_OVERLAP */
...
#pragma omp parallel for
...
#if defined(USE_CTX_OVERLAP)
  /* Double-buffer ONLY the prefetched inputs (seq + score); gaps and outputs are
   * single-buffered. A dedicated context carries the prefetch stream. */
  const index_t agg_bufsz = seq_data->match->length + 2;
  codon_t *buf_main[2]  = {(codon_t *)malloc(sizeof(codon_t) * agg_bufsz),
                           (codon_t *)malloc(sizeof(codon_t) * agg_bufsz)};
  codon_t *buf_match[2] = {(codon_t *)malloc(sizeof(codon_t) * agg_bufsz),
                           (codon_t *)malloc(sizeof(codon_t) * agg_bufsz)};
  score_t *buf_score[2] = {(score_t *)malloc(sizeof(score_t) * agg_bufsz),
                           (score_t *)malloc(sizeof(score_t) * agg_bufsz)};
  score_t *agg_maingap  = (score_t *)malloc(sizeof(score_t) * agg_bufsz);
  score_t *agg_matchgap = (score_t *)malloc(sizeof(score_t) * agg_bufsz);
  score_t *agg_wscore   = (score_t *)malloc(sizeof(score_t) * agg_bufsz);
  score_t *agg_wmaingap = (score_t *)malloc(sizeof(score_t) * agg_bufsz);
  score_t *agg_wmatchgap= (score_t *)malloc(sizeof(score_t) * agg_bufsz);
  shmem_ctx_t pfetch;
  if (CTX_CREATE(&pfetch) != 0) pfetch = SHMEM_CTX_DEFAULT; /* fall back to default ctx */
#elif defined(USE_MANUAL_AGG)
  /* local scratch buffers, sized to the longest possible anti-diagonal */
  const index_t agg_bufsz = seq_data->match->length + 2;
  codon_t *agg_main     = (codon_t *)malloc(sizeof(codon_t) * agg_bufsz);
  codon_t *agg_match    = (codon_t *)malloc(sizeof(codon_t) * agg_bufsz);
  score_t *agg_score    = (score_t *)malloc(sizeof(score_t) * agg_bufsz);
  score_t *agg_maingap  = (score_t *)malloc(sizeof(score_t) * agg_bufsz);
  score_t *agg_matchgap = (score_t *)malloc(sizeof(score_t) * agg_bufsz);
  score_t *agg_wscore   = (score_t *)malloc(sizeof(score_t) * agg_bufsz);
  score_t *agg_wmaingap = (score_t *)malloc(sizeof(score_t) * agg_bufsz);
  ...
  */
...
#ifdef USE_CTX_OVERLAP
  /* Prime the pipeline: make the by-hand idx=0,1 writes globally visible, then
   * prefetch the first wavefront diagonal's reusable inputs (idx=2 -> buf[0]). */
  BARRIER_ALL();
  {
    index_t ls2, le2;
    ctx_diag_range(2, seq_data->match->length, local_main_start, local_main_end, &ls2, &le2);
    if (ls2 <= le2)
      ctx_prefetch_inputs(pfetch, 2, ls2, le2, main_seq, match_seq, score_matrix,
                          buf_main[0], buf_match[0], buf_score[0]);
  }
#endif
  for(index_t idx=2; idx < seq_data->match->length * 2 - 1; idx++) {
...
#if defined(USE_CTX_OVERLAP)
    {
      const index_t cur = idx & 1, nxt = (idx + 1) & 1;
      const index_t rgap = (idx - 1) % 2;

      /* This diagonal's prefetched seq+score (buf[*][cur]) have now landed. */
      CTX_QUIET(pfetch);

      /* depth-1 gaps: produced by the previous diagonal, fetched now (blocking) -
       * the only inputs still on the critical path. */
      if (local_start <= local_end) {
        agg_get_matrix(main_gap_matrix,  rgap, local_start - 1,       local_end - 1,         agg_maingap);
        agg_get_matrix(match_gap_matrix, rgap, idx - local_end - 1,   idx - local_start - 1, agg_matchgap);
      }

      /* Prefetch idx+1's reusable inputs on `pfetch` so their latency overlaps the
       * compute below. idx+1's score row (idx-1)%3 is already final (diagonal idx-1
       * completed at this iteration's BARRIER_ALL). */
      if (idx + 1 < seq_data->match->length * 2 - 1) {
        index_t ls1, le1;
        ctx_diag_range(idx + 1, seq_data->match->length, local_main_start, local_main_end, &ls1, &le1);
        if (ls1 <= le1)
          ctx_prefetch_inputs(pfetch, idx + 1, ls1, le1, main_seq, match_seq, score_matrix,
                              buf_main[nxt], buf_match[nxt], buf_score[nxt]);
      }

      if (local_start <= local_end) {
        const index_t ls = local_start, le = local_end;
        const codon_t *cm = buf_main[cur];
        const codon_t *cx = buf_match[cur];
        const score_t *cs = buf_score[cur];
        for (index_t antidiagonal = ls; antidiagonal <= le; antidiagonal++) {
          m = antidiagonal;
          n = idx - m;
          current_main  = cm[m - ls];
          current_match = cx[le - m];
          G = cs[m - ls];
          E = agg_maingap[m - ls];
          F = agg_matchgap[le - m];
          ...
          cmp_b = G - gapFirst;
          cmp_c = F - gapExtend;
          agg_wscore[m - ls]    = new_score;
          agg_wmaingap[m - ls]  = cmp_a > cmp_b ? cmp_a : cmp_b;
          agg_wmatchgap[le - m] = cmp_c > cmp_b ? cmp_c : cmp_b;
        }
        /* scatter outputs (default context) and complete them before the barrier */
        agg_put_matrix(score_matrix,     idx % 3, ls,       le,       agg_wscore);
        agg_put_matrix(main_gap_matrix,  idx % 2, ls,       le,       agg_wmaingap);
        agg_put_matrix(match_gap_matrix, idx % 2, idx - le, idx - ls, agg_wmatchgap);
        QUIET();
      }
    }
#elif defined(USE_MANUAL_AGG)
    if (local_start <= local_end) {
      const index_t ls = local_start, le = local_end;
      const index_t rscore = (idx - 2) % 3, rgap = (idx - 1) % 2;
      /* one bulk SHORT_GET per owning PE for each array's contiguous range */
      agg_get_seq(main_seq,  ls,         le,         agg_main);              /* main_seq[m]            */
      agg_get_seq(match_seq, idx - le,   idx - ls,   agg_match);             /* match_seq[idx-m]       */
      agg_get_matrix(score_matrix,     rscore, ls - 1,     le - 1,    agg_score);    /* score[(idx-2)%3][m-1]  */
      agg_get_matrix(main_gap_matrix,  rgap,   ls - 1,     le - 1,    agg_maingap);  /* main_gap[(idx-1)%2][m-1]*/
      agg_get_matrix(match_gap_matrix, rgap,   idx - le-1, idx - ls-1,agg_matchgap); /* match_gap[(idx-1)%2][n-1]*/

      for (index_t antidiagonal = ls; antidiagonal <= le; antidiagonal++) {
        m = antidiagonal;
        n = idx - m;
        current_main  = agg_main[m - ls];
        current_match = agg_match[le - m];
        G = agg_score[m - ls];
        E = agg_maingap[m - ls];
        F = agg_matchgap[le - m];
      ...
      /* one bulk SHORT_PUT per owning PE for each result range */
      agg_put_matrix(score_matrix,     idx % 3, ls,       le,       agg_wscore);
      agg_put_matrix(main_gap_matrix,  idx % 2, ls,       le,       agg_wmaingap);
      agg_put_matrix(match_gap_matrix, idx % 2, idx - le, idx - ls, agg_wmatchgap);
      QUIET();
...
#endif /* USE_MANUAL_AGG */
  }

#if defined(USE_CTX_OVERLAP)
  CTX_DESTROY(pfetch);
  free(buf_main[0]);  free(buf_main[1]);
  free(buf_match[0]); free(buf_match[1]);
  free(buf_score[0]); free(buf_score[1]);
  free(agg_maingap);  free(agg_matchgap);
  free(agg_wscore);   free(agg_wmaingap); free(agg_wmatchgap);
#elif defined(USE_MANUAL_AGG)
  free(agg_main); free(agg_match); free(agg_score); free(agg_maingap);
  free(agg_matchgap); free(agg_wscore); free(agg_wmaingap); free(agg_wmatchgap);
#endif
