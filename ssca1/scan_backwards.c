...
#define CLEANUP_AND_RETURN() do { \
      ...
      if(s == goal)
      {
        //discard if start is too close to a better sequence
	      ...
	      {
          compare_a = di > A->bestStarts[0][r] ? di - A->bestStarts[0][r] : A->bestStarts[0][r] - di;
          compare_b = dj > A->bestStarts[1][r] ? dj - A->bestStarts[1][r] : A->bestStarts[1][r] - dj;
