...
# SSCA1 source layout
# ---------------------------------------------------------------------------
SSCA1_DIR=/mnt/DISCL/home/jcowles/MLIR_testing/benchmarks/ssca1
...
# Build directories
# ---------------------------------------------------------------------------
BUILD_DIR="${BUILD_DIR:-/mnt/DISCL/home/jcowles/MLIR_testing/benchmarks/ssca1/build/default}"
mkdir -p \
  "${BUILD_DIR}/cir" \
  "${BUILD_DIR}/flat_cir" \
...
# Source-level manual message aggregation in pairwise_align.c (USE_MANUAL_AGG=1).
if [[ "${USE_MANUAL_AGG:-0}" == "1" ]]; then
  CFLAGS+=( -DUSE_MANUAL_AGG )
fi
# Context-based communication/computation overlap (USE_CTX_OVERLAP=1); builds on
# manual aggregation, so it implies -DUSE_MANUAL_AGG for the shared strip-miners.
if [[ "${USE_CTX_OVERLAP:-0}" == "1" ]]; then
  CFLAGS+=( -DUSE_CTX_OVERLAP -DUSE_MANUAL_AGG )
fi
...
compile_file() {
  ...
  # Step 2: NEW PIPELINE — do NOT flatten here. Keep structured cir.for loops so
  # the OpenSHMEM loop passes (inline-comm-helpers, scalar-to-bulk) can see them.
  # Flatten/abi-lowering happen later, inside shmem-mlir-opt, before --cir-to-llvm.
  ${CIR_OPT} \
    --cir-goto-solver \
    "${BUILD_DIR}/cir/${base}.cir" \
    -o "${BUILD_DIR}/flat_cir/${base}.flat.cir" 2>&1 || return 1

  # Scalar-to-bulk aggregation is toggleable so the same pipeline can build a
  # non-aggregated control (USE_SCALAR_TO_BULK=0) and an aggregated optimized
  # build (USE_SCALAR_TO_BULK=1, plus STB_FLAG="=assume-independent=true").
  local SCALAR_TO_BULK_ARGS=()
  if [[ "${USE_SCALAR_TO_BULK:-0}" == "1" ]]; then
    SCALAR_TO_BULK_ARGS+=("--openshmem-scalar-to-bulk${STB_FLAG:-}")
  fi

  # Steps 3-5: CIR → OpenSHMEM → (loop passes) → flatten/abi → Lower to LLVM.
  ${SHMEM_OPT} \
