...
envs="MV2_HOMOGENEOUS_CLUSTER=1 MV2_SMP_USE_CMA=0"
...
do
    ...
    for i in 10.1.1.82 10.1.1.81; do
        echo "$i:$ppn" >> $hostfile
    done
    ...
    for mesh in 4 8 16 32; do
        ...
        touch $configfile
        for i in 1 2; do
            echo "-n $ppn : $PWD/osh_nb $mesh $mesh $proc_grid 2 $ppn" >> \
                $configfile
        done
        ...
        for q in `seq 1 5`; do
          #  set -x
          if [[ $q == 1 ]] ; then
              mpirun_rsh -np $nprocs --hostfile $hostfile $envs \
                  $PWD/osh_nb $mesh $mesh $proc_grid 2 $ppn \
                  2>/dev/null | head -n2
          else
              mpirun_rsh -np $nprocs --hostfile $hostfile $envs \
                  $PWD/osh_nb $mesh $mesh $proc_grid 2 $ppn \
                  2>/dev/null | head -n2 | tail -n1
          fi
