        ...
        if (work_id < work_units){
...
#endif
                shmemx_secure_get_nbi(SHMEM_CTX_DEFAULT, local_d, win + target_offset_d + disp_d, elements_in_tile*sizeof(double), target_rank_d);
            ...
            *get_start_2 = shmem_getwtime();
            if (target_rank_e == rank) {
                ...
                printf("Rank %d] local_e: Get size: %ld\n", rank, elements_in_tile *sizeof(double)); 
#endif
                shmemx_secure_get_nbi(SHMEM_CTX_DEFAULT, local_e, win + target_offset_e + disp_e, elements_in_tile*(sizeof(double)), target_rank_e);
...
//    MPI_Info_free(&win_info);
   ...
   // shmem_free(local_c);
    ...
    shmem_free(get_counter);
    shmem_free(accum_counter);
    shmem_free(tot_get_count);
    shmem_free(tot_accum_count);
    shmem_free(total_get_start);
    shmem_free(total_get_end);
    shmem_free(total_accum_start);
    shmem_free(total_accum_end);
