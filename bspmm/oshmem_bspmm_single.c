...
//                    comm_world, &win_mem, &win);
 ...
 //   MPI_Win_lock_all(MPI_MODE_NOCHECK, win_counter);
    ...
    do {
        ...
        work_id = shmem_int_atomic_fetch_add(counter_win_mem, one, 0);
...
//        MPI_Win_flush(0, win_counter);
        ...
        if (target_rank_a == rank) {
            ...
            /* get tile A */
            shmem_double_get(local_a, win+disp_a+target_offset_a, elements_in_tile, target_rank_a);
            shmem_quiet();
      ...
      //  shmem_clear_lock(&lock2);
        ...
        if (target_rank_a == rank) {
          ...
          //  shmem_clear_lock(&lock1);
            ...
            shmem_quiet();
#if FINE_TIME
            t_get += (shmem_getwtime() - t_start);
            t_start = shmem_getwtime();
#endif
        //    MPI_Win_flush(target_rank_a, win);
#if FINE_TIME
            t_get_flush += (shmem_getwtime() - t_start);
#endif       
        }
